...
CSV.open("#{filename}_mounted.csv","w") do |csv1|
CSV.open("#{filename}_not_in_openstack.csv","w") do |csv2|
CSV.open("#{filename}_missing_object.csv","w") do |csv3|
    ...
    if not row[8].nil? or result!=true
      sum2+=1
        ...
        if row_num1!=0
          ...
          rs3.each do |row3|
            row_num3+=1
            csv2 << [row3['code'],row3['old_peel_new']]
          end
