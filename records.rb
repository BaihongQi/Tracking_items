...
puts filename
CSV.open("#{filename}_mods_records.csv","w",:write_headers=> true,:headers => ["title","Peel no.","version of","mods_record"]) do |csv1|
CSV.open("#{filename}_parent.csv","w",:write_headers=> true,:headers => ["title","Peel no."]) do |csv2|
